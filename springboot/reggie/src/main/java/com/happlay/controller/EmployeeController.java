...
public class EmployeeController {
    ...
    private R<Employee> login(HttpServletRequest request, @RequestBody Employee employee) {
        ...
        request.getSession().setAttribute("employee", emp.getId());  // 用于在 HTTP 会话中设置一个名为 "employee" 的属性，其值为 emp.getId()。
    ...
    public R<String> logout(HttpServletRequest request) {
        ...
        // request.getSession().setAttribute("employee", emp.getId());那这句话的意思是每次有不同的id，
        // 但是属性名都是employee，那我清除session的时候不会把不同id全清空吗
        // 可能每台电脑上的浏览器只需要登录一个账户信息
...
//
//        // 获取当前登录用户的id，getAttribute返回的是Object类型
...
//        employee.setUpdateUser(empId);
    ...
    @GetMapping("/{id}")  // /employee/121414141 用@PathVariable设置路径变量
